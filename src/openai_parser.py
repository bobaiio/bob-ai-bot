...
class OpenAIParser:
    ...
    def get_response(self, userid, context_messages):
        LoggingManager.debug("Get Bob AI response for user: %s" % userid, "OpenAIParser")
        ...
        try:
            ...
            LoggingManager.error("Bob AI request for user %s with error: %s" % (userid, str(e)), "OpenAIParser")
            return ("Oops, something went wrong with Bob AI. Please try again later.", 0)

    def speech_to_text(self, userid, audio_file):
        LoggingManager.debug("Get Bob AI Speech to Text for user: %s" % userid, "OpenAIParser")
        # transcript = openai.Audio.transcribe("whisper-1", audio_file, language="zh")
        try:
            transcript = openai.Audio.transcribe("whisper-1", audio_file)
        except Exception as e:
            LoggingManager.error("Bob AI Speech to Text request for user %s with error: %s" % (userid, str(e)), "OpenAIParser")
            return ""
        return transcript["text"]

    def image_generation(self, userid, prompt):
        LoggingManager.debug("Get Bob AI Image Generation for user: %s" % userid, "OpenAIParser")
