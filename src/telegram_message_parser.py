...
class TelegramMessageParser:
    ...
    def add_handlers(self):
        ...
        if ConfigLoader.get("image_generation", "enable_dalle"):
            self.bot.add_handler(CommandHandler("image", self.image_generation))
        ...
        self.bot.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), self.chat_text))
        self.bot.add_handler(CommandHandler("ask", self.chat_text_command))
    ...
    async def image_generation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # message = update.effective_message.text.replace("/image", "")
        message = " ".join(context.args)
        ...
        if not allowed:
            ...
            results = [
                InlineQueryResultArticle(
                    id = str(uuid4()),
                    title = "Chat💬",
                    description = "Get a response from Bob AI (It's a beta feature, no context ability yet)",
